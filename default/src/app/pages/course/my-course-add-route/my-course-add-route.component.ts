...
@Component({
  selector: 'app-my-course-add-route',
  templateUrl: './my-course-add-route.component.html',
  styleUrls: ['./my-course-add-route.component.scss']
})
export class MyCourseAddRouteComponent implements OnInit {
  ...
  onFileSelect(event) {
    ...
    this.courseService.addCourse(formData).subscribe((res: any) => {
      ...
      this.router.navigate(['/course/addMyCourse/lesson/chapter', res.id]);
    }, error => {
